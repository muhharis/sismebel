...
<div class="row">
	<div class="col-lg-8">
		<h1 class="mt-4">Manage Data User</h1><hr/>
		<?php
		if(isset($_GET['op'])){
			$op = $_GET['op'];
			if($op=="tambah"){
				...
				<form method="post" action="<?php echo basename(__FILE__, '.php');?>?op=save">
					...
					<input type="text" name="e" class="form-control" placeholder="No. Telp" maxlength="15" style="width:100%" required/><br/><br/>
		...
		?>
		<b style="font-size:16pt;">Data User</b> <a href="<?php echo basename(__FILE__, '.php');?>?op=tambah" style="text-decoration:none;font-size:16pt;float:right;">Tambah Data User</a><br/><br/>
		<table id="myTable" class="table table-striped" style="border:1px solid #000;">
			...
			<tbody>
			<?php
			$query = "SELECT * FROM `tb_login` WHERE `level` = 'user'";
			$hasil = mysqli_query($db, $query);
